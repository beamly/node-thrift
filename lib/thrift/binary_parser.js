...
var int64 = require('./int64_parser');
...
p.encodeInt = function( data, bits, signed, forceBigEndian ){
  var padBytes = 0;
  if(bits == 64 && data < 0) {
      // Javascript loses precision at large 64-bit integers
      // this causes max (as calculated below) to be incorrect
      // causing the code below to enter an infinite loop for negative
      // numbers.
      // To avoid this, we remove the same precision (2 bytes) from the
      // calculation, then pad the result back to the expected number of bytes
      padBytes = 2;
      bits -= 16;
  }
